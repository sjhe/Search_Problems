...
public class Search {
	...
	public String IterativeDeepeningTreeSearch() {
		int depth = 0 ;
		while(true){
			String node = TreeSearchDepthLimited(new FrontierLIFO() , depth) ;
			if( node != null){
				return node;
			}else{
				depth++;
	...
	public String IterativeDeepeningGraphSearch() {
		for(int depth = 0 ; ; depth++){
			// System.out.println("Depth = " + depth + "Count = " + cnt);
			String node = GraphSearchDepthLimited(new FrontierLIFO() , depth);
			if( node != null){
				return node;
			}
		}
	...
	//A star graph search without consistent heuristic
	public String A_StarGraphSearch() {
		return AGraphSearch(new FrontierFIFO());
	}
	...
	private String AGraphSearch(Frontier frontier) {
		cnt = 0; 
		double cost = 0;
		node_list = new ArrayList<Node>();
		...
		Map<Object, Double> explored = new HashMap<Object, Double>(); //empty set
		frontier.insert( initialNode );
		while(true) {
			...
			if(!explored.isEmpty()){
				if(explored.get(node.state) ==  null){
					// System.out.println("state : " + node.state + "has cost null");
					cost = -1;
				}else{
					cost = explored.get(node.state);
					// System.out.println("State : " + node.state  + "path_cost : " + node.path_cost);
				}

			}
			if(node.path_cost >= 0){
				// System.out.println("Cost = " + cost  + "path_cost : " + node.path_cost);
